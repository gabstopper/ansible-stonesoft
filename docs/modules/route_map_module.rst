...
* Route Maps are used by the BGP configuration to allow refined BGP based policies for the NGFW. Route maps can be applied to announced networks on the engine BGP configuration or to BGPPeering elements. This module provides the ability to create a route map policy and route map rules. To view an existing route map, use route_map_facts.
